...
class WriteChunk:
    ...
    async def upsert_rows(self, tname, table):
    ...
    async def delete_rows(self, tname, table):
        ...
        async with self.conn.cursor() as cursor:
            await cursor.execute(delete_sql.format(t=tname, columns=c, v=values))

    async def insert_rows(self, tname, table):
        sx, tx = WriteChunk._split_table_name(tname)
...
delete from {fqtn} where ({pknames}) in (select * from staging)""".format(
        ...
        async with conn.cursor() as cursor:
            ...
            if len(table.deleted_keys) > 0:
                ...
                await cursor.execute(my_delete, {"keys": tuple(table.deleted_keys)})
            ...
            if len(rows1) > 0:
                ...
                if len(cols_no_pk) > 0:
                    ...
                    await cursor.execute(my_update)
                # insert
                my_insert = insert.replace("/*REPRESENTED*/", mogrifications)
                await cursor.execute(my_insert)
